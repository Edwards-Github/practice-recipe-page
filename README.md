...
| **Text overflowing on mobile**: Fixed `width: 30%` caused content to overflow on narrow screens | Use `width: 100%` combined with `max-width: 700px` so the card flexibly fills the screen but never exceeds 700px. width: 100% with max-width: 700px doesn't actually set a minimum width. The minimum width comes from the width: 30% in container class. |
